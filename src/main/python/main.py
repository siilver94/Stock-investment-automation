...
import kakao_chatbot1

def dbgout(message):
    """인자로 받은 문자열을 파이썬 셀과 카톡으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message   
    kakao_chatbot1.kakao_sendtext(strbuf)   
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_low) * 0.1 #지난날의 고가와 저가를 계산에서 K만큼 곱한 값
        return target_price
...
def buy_etf(code): #변동성 돌파 전략 함수
    ...
    try:
        ...
        #이동평균선: 말그대로 주가의 이동평균을 구해서 평균값을 이은 선
        ...
        if current_price > target_price and current_price > ma5_price \
            ...
            #dbgout('\n최유리 FoK 매수 ->'+ str(stock_name) + str(code) + str(buy_qty)+ '->'+ str(ret))
            if ret == 4:
                ...
                return False
            time.sleep(2)
            printlog('현금주문 가능금액 :', str(buy_amount))
            #dbout('현금주문 가능금액 :'+ buy_amount)
            stock_name, bought_qty = get_stock_balance(code)
            printlog('get_stock_balance :', stock_name, stock_qty)
            if bought_qty > 0:  #구매 수량이 0 보다 크면
                bought_list.append(code) #bought_list 에 현재 코드를 더해줘서 이미 산 종목인 것을 표시
                #현재 어떤걸 얼마나 샀는지 메세지 전송
...
if __name__ == '__main__': 
    try:
        #자동 매매를 하려 하는 종목 코드
        #TIGER소프트웨어, TIGER중소형성장, HANARO_Fn친환경에너지,KBSTAR게임테마
        symbol_list = ['A157490','A292130','A381570','A300640']
        bought_list = []     # 매수 완료된 종목 리스트
        target_buy_count = 4 # 매수할 종목 수 symbol_list와 같게 하기
        buy_percent = 0.25   #매수할 종목들을 전체 가용 자금에서 몇퍼 센트 살껀지(균등하게)
        printlog('check_creon_system() :', check_creon_system())  #크레온 접속 점검
        time.sleep(1)
        stocks = get_stock_balance('ALL')      # 보유한 모든 종목 조회
        total_cash = float(get_current_cash())   # 100% 증거금 주문 가능 금액 조회
        ...
        dbgout('시작 시간 :'+ str(datetime.now().strftime('%m/%d %H:%M:%S')))
        soldout = False

        while True:
            t_now = datetime.now() #시간 계산
            t_9 = t_now.replace(hour=9, minute=0, second=0, microsecond=0)
            t_start = t_now.replace(hour=9, minute=5, second=0, microsecond=0) #9시 5분 시작
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0)# 팔기 시작하는 시간 15시 15분
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0)# 끝내는 시간 15시 20분
            today = datetime.today().weekday() #오늘 날짜 
            if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료
                printlog('Today is', 'Saturday.' if today == 5 else 'Sunday.')
                sys.exit(0)
            if t_9 < t_now < t_start and soldout == False:#현재시간이 9시나 9시 5분 사이일떄 모든 주식 팔기
                soldout = True
                sell_all()
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                for sym in symbol_list:
                    if len(bought_list) < target_buy_count: #목표한 종목수보다 못샀으면
                        buy_etf(sym) #가격이 지금 매수할 타이밍이 맞는지 검사
                        time.sleep(1)
                if t_now.minute == 30 and 0 <= t_now.second <= 5:  #30분마다 현재 잔고가 얼마고 얼마 벌었고
